...
const Header = () => {
  ...
  const renderNavLinks = (isMobile: boolean) => (
    <>
      ...
      <Link
        href="/"
        className={`${isMobile ? 'block py-2 px-4' : ''} text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white cursor-pointer`}
      >
        Reservar um Recurso
      </Link>
      <Link
  ...
  const renderAuthLinks = (isMobile: boolean) => (
    <>
      {user ? (
        <div
          className={`${isMobile ? 'border-t mt-2 pt-2' : 'border-l pl-4 ml-4'} border-gray-200 dark:border-gray-700 flex items-center ${isMobile ? '' : 'space-x-4'}`}
        >
          <span
            className={`${isMobile ? 'block py-2 px-4' : ''} text-gray-600 dark:text-gray-300`}
          >
            Bem-vindo(a),{' '}
            <Link
              href="/profile/me"
